...
export default function CourseScreen() {
  ...
  const [userPerformance, setUserPerformance] = useState({ avgResponseTime: 15, correctRate: 0.7 });

  const courseContent = {
    videos: [
      { id: 'v1', title: 'Introduction to Machine Learning', duration: '12:30', completed: false },
      { id: 'v2', title: 'Types of ML Algorithms', duration: '18:45', completed: false },
      { id: 'v3', title: 'Data Preprocessing', duration: '15:20', completed: false }
    ],
    readings: [
      { id: 'r1', title: 'ML Fundamentals PDF', pages: 25, completed: false },
      { id: 'r2', title: 'Algorithm Comparison Chart', pages: 8, completed: false },
      { id: 'r3', title: 'Case Study: Netflix Recommendations', pages: 12, completed: false }
    ],
    quizzes: [
      { id: 'q1', title: 'Week 1 Quiz', questions: 10, completed: false },
      { id: 'q2', title: 'Algorithm Types Quiz', questions: 8, completed: false }
    ]
  };
  ...
  const ResourceItem = ({ item, type }: { item: any; type: string }) => (
    <TouchableOpacity 
      ...
      onPress={() => type === 'quiz' ? router.push('/quiz') : toggleComplete(item.id)}
    >
      <View className="flex-row items-center justify-between">
        <View className="flex-1">
          <View className="flex-row items-center mb-2">
            ...
            <Text className="flex-1 font-semibold text-gray-900">{item.title}</Text>
          </View>
          <Text className="text-sm text-gray-600">
            {type === 'video' ? item.duration : 
             type === 'reading' ? `${item.pages} pages` : 
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 20 }}>
        ...
        <View className="px-6 pt-4 pb-6">
          <View className="flex-row items-center justify-between mb-4">
            ...
            <Text className="text-lg font-bold text-gray-900">Machine Learning Basics</Text>
            <View />
          </View>
          ...
          <View className="p-4 bg-white shadow-lg rounded-2xl">
            ...
            <View className="w-full h-2 bg-gray-200 rounded-full">
              <View className="h-full rounded-full bg-emerald-500" style={{ width: '25%' }} />
            </View>
          </View>
        </View>
        ...
        <View className="px-6">
          {activeTab === 'overview' && (
            <View className="p-6 bg-white shadow-lg rounded-2xl">
              <Text className="mb-4 text-xl font-bold text-gray-900">About This Course</Text>
              ...
              <Text className="mb-2 font-semibold text-gray-700">What you'll learn:</Text>
              <View className="gap-2">
                <Text className="text-gray-700">• Core ML concepts and terminology</Text>
                <Text className="text-gray-700">• Popular algorithms and their applications</Text>
                <Text className="text-gray-700">• Data preprocessing techniques</Text>
          ...
          {activeTab === 'resources' && (
            <View>
              <Text className="mb-4 text-lg font-bold text-gray-900">📹 Videos</Text>
              {courseContent.videos.map(video => (
                <ResourceItem key={video.id} item={video} type="video" />
              ))}
              
              <Text className="mt-6 mb-4 text-lg font-bold text-gray-900">📚 Readings</Text>
              {courseContent.readings.map(reading => (
                <ResourceItem key={reading.id} item={reading} type="reading" />
              ))}
              
              <Text className="mt-6 mb-4 text-lg font-bold text-gray-900">🧠 Quizzes</Text>
          ...
          {activeTab === 'quiz' && (
            <View className="p-6 bg-white shadow-lg rounded-2xl">
              <Text className="mb-4 text-xl font-bold text-gray-900">Ready for the Quiz?</Text>
              <Text className="mb-6 text-gray-700">
                Test your knowledge with 10 adaptive questions that adjust to your skill level.
              </Text>
              <TouchableOpacity 
                className="py-4 bg-emerald-500 rounded-xl"
                onPress={() => router.push('/quiz')}
              >
                <Text className="text-lg font-bold text-center text-white">Start Quiz</Text>
              </TouchableOpacity>
            </View>
          )}
        </View>
